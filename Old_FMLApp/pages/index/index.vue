<template>
	...
	@click-left="clickLeft" @click-right="clickRight">
		...
		<view style="display: flex;justify-content: center;align-items: center;border-radius: 4px;margin-left: -46upx;height: 60upx;margin-top: 12upx;color: #CCCCCC;background: #F7F7F7;" @tap="openSearch">
			<view class="iconfont icon-sousuo" style="margin-right: 6upx;"></view>搜索帖子
		</view>
		<!-- 右边图标 -->
		<block slot="right">
	...
	<swiper class="swiper-box" :style="getHeight" :current="tabIndex" @change="tabChange" :duration="120">
	...
	export default {
		...
		methods: {
			...
			async getList(){
				...
				let page = this.newslist[currentIndex].page
				
				let url = `/postclass/${this.tabBars[this.tabIndex].id}/post/${page}`;
			...
			tabtap(index){
				this.tabIndex=index;
				// 防止多次加载
				if (this.newslist[index].firstload) return;
				// #ifdef MP-ALIPAY
				this.getList();
				// #endif
			},
			...
			tabChange(e){
				...
				// 防止多次加载
				if (this.newslist[this.tabIndex].firstload) return;
				this.getList();
			}
		},
	}

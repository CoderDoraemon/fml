<template>
	<view>
		...
		<!-- #endif -->
		<!-- 搜索历史 -->
		<view class="py-2" v-if="!searchtext">
			<view class="font-md main-light-dark px-2 mb-2">搜索历史</view>
			<view class="flex flex-wrap">
				
				<view class="border mx-2 px-2 mb-2 rounded animated faster main-light-dark" hover-class="main-bg-color text-white pulse border-0" v-for="(item,index) in history" :key="index" @click="searchHistory(item)">{{item}}</view>

			</view>
		</view>
		
		
		<template v-if="list.length>0 && searchtext">
	...
	export default {
		...
		data() {
			return {
				...
				page:1,
				searchType:"post",
				history:['uni-app第二季实战商城类app和小程序','vue','vue实战','uni-app实战','vuex实战','uni-app第三季实战微信app和小程序开发','vuex','依赖注入','123']
			};
		},
		// 监听原生标题导航按钮点击事件(取消按钮)

<template>
	<view>
		...
		<!-- #endif -->
		<!-- 背景图 + 用户基本信息 -->
		<user-space-head :userinfo="userinfo" :spacedata="spacedata" @update="updateGuanZhu"></user-space-head>
		<!-- 统计 -->
		...
		<block v-for="(item,index) in tablist" :key="index">
			...
			<template v-else-if="tabIndex==index">
				...
				<template v-else>
					<!-- 无内容默认 -->
					<view style="font-size: 50upx;font-weight: bold;color: #CCCCCC;
					padding-top: 100upx;" class="u-f-ajc">
						靓仔，空空如也~
					</view>
				</template>
			</template>
		...
		<uni-popup ref="popup" type="top" @hidePopup="show = false">
			<view class="u-f-ajc py-2 animated faster font-md border-bottom border-light-secondary" hover-class="pulse main-color" @tap="lahei">
				<text class="iconfont icon-sousuo mr-2"></text>
				{{userinfo.isblack ? '移除黑名单' : '加入黑名单'}}
			</view>
			<view v-show="!userinfo.isblack" 
			class="u-f-ajc py-2 animated faster font-md"
			hover-class="pulse main-color" @tap="goToChat">
